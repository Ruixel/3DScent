				...
				switch (sig) {
					...
					case bmhd_sig: {
						...
						if (bmheader->raw_data) {
							...
							bmheader->raw_data = malloc (bmheader->w * bmheader->h);
							if (!bmheader->raw_data)
								return IFF_NO_MEM;
						}
...
int convert_ilbm_to_pbm(iff_bitmap_header *bmheader)
{
	...
	new_data = malloc (bmheader->w * bmheader->h);
...
int open_fake_file(char *ifilename,FFILE *ffile)
{
	...
	ffile->data = malloc (ffile->length);
...
void copy_iff_to_grs(grs_bitmap *bm,iff_bitmap_header *bmheader)
{
	...
	bm->bm_w = bmheader->w;
	bm->bm_h = bmheader->h;
	bm->bm_type = (ubyte)bmheader->type;
