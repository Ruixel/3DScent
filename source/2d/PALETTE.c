...
//	outp( 0x3c8, 0 );
	p=gr_palette;
	for (i=0; i<256; i++ )	{
		nr = (int)(*p++) + r + gr_palette_gamma;
		if (nr<0) nr=0;
		else if (nr>63) nr=63;

//		outp( 0x3c9, temp );
		ng = (int)(*p++) + g + gr_palette_gamma;
		if (ng<0) ng=0;
		else if (ng>63) ng=63;

//		outp( 0x3c9, temp );
		nb = (int)(*p++) + b + gr_palette_gamma;
		if (nb<0) nb=0;
		else if (nb>63) nb=63;
...
//		outp( 0x3c9,c);
	...
	*/
	for (i=0, j=0; i<256; i++ )	{
		r = pal[j] + gr_palette_gamma;
		if ( r > 63 ) r = 63;
 		gr_current_pal[j] = pal[j];	j++;

		g = pal[j] + gr_palette_gamma;
		if ( g > 63 ) g = 63;
 		gr_current_pal[j] = pal[j];	j++;

		b = pal[j] + gr_palette_gamma;
		if ( b > 63 ) b = 63;
 		gr_current_pal[j] = pal[j];	j++;
