...
void scale_inbitmap(grs_bitmap *dst, grs_bitmap *src )
{
	...
	clipped_u0 = 0; clipped_v0 = 0;
	clipped_u1 = i2f(src->bm_w-1); clipped_v1 = i2f(src->bm_h-1);

	clipped_x0 = 0; clipped_y0 = 0;
	clipped_x1 = i2f(dst->bm_w)-fl2f(.5); clipped_y1 = i2f(dst->bm_h)-fl2f(.5);
	
	dx0 = 0; dx1 = f2i(clipped_x1);
	dy0 = 0; dy1 = f2i(clipped_y1);
...
//mprintf( 0, "(%.2f,%.2f) to (%.2f,%.2f) using (%.2f,%.2f) to (%.2f,%.2f)\n", f2fl(clipped_x0), f2fl(clipped_y0), f2fl(clipped_x1), f2fl(clipped_y1), f2fl(clipped_u0), f2fl(clipped_v0), f2fl(clipped_u1), f2fl(clipped_v1) );
	...
	if ( src->bm_flags & BM_FLAG_RLE )	{
		if ( (dtemp < (f2i(clipped_x1)-f2i(clipped_x0))) && (dtemp>0) )
			scale_up_bitmap_rle(src, dst, dx0, dy0, dx1, dy1, clipped_u0, clipped_v0, clipped_u1, clipped_v1  );
		else
			scale_bitmap_c_rle(src, dst, dx0, dy0, dx1, dy1, clipped_u0, clipped_v0, clipped_u1, clipped_v1  );
	} else {
		if ( (dtemp < (f2i(clipped_x1)-f2i(clipped_x0))) && (dtemp>0) )
			scale_up_bitmap(src, dst, dx0, dy0, dx1, dy1, clipped_u0, clipped_v0, clipped_u1, clipped_v1  );
		else
			scale_bitmap_c(src, dst, dx0, dy0, dx1, dy1, clipped_u0, clipped_v0, clipped_u1, clipped_v1  );
	}
}
