...
vms_vector *rod_point_list[] = {&rod_points[0].p3_vec,&rod_points[1].p3_vec,&rod_points[2].p3_vec,&rod_points[3].p3_vec};
...
ITCM_CODE int calc_rod_corners(vms_vector *bot_point,fix bot_width,vms_vector *top_point,fix top_width)
{
	...
	vm_vec_sub(&delta_vec,bot_point,top_point);
	...
	vm_vec_copy_normalize(&top,top_point);
	...
	vm_vec_add(&rod_points[0].p3_vec,top_point,&tempv);
	vm_vec_sub(&rod_points[1].p3_vec,top_point,&tempv);

	vm_vec_copy_scale(&tempv,&rod_norm,bot_width);
	tempv.z = 0;

	vm_vec_sub(&rod_points[2].p3_vec,bot_point,&tempv);
	vm_vec_add(&rod_points[3].p3_vec,bot_point,&tempv);
...
ITCM_CODE void g3_draw_rod_tmap(grs_bitmap *bitmap,vms_vector *bot_point,fix bot_width,vms_vector *top_point,fix top_width,fix light)
{
	...
	g3_draw_tmap_tex(4,rod_point_list,uvl_list,bitmap);
}
