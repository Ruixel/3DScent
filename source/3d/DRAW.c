...
void (*flat_drawer_ptr)(int nv,fix *vertlist) = gr_upoly_tmap;
...
bool g3_draw_line(g3s_point *p0,g3s_point *p1)
{
	...
	return (bool) gr_line(p0->p3_sx,p0->p3_sy,p1->p3_sx,p1->p3_sy);
}
...
/*
ITCM_CODE bool do_facing_check(vms_vector *norm,g3s_point **vertlist,vms_vector *p)
{
	if (norm) {		//have normal
...
bool must_clip_flat_face(int nv,g3s_codes cc)
{
	...
	if (nv>0 && !(cc.or&CC_BEHIND) && !cc.and) {
		...
		for (i=0;i<nv-1;i++) {
			g3s_point *p = bufptr[i];
			g3s_point *p2 = bufptr[i+1];

			gr_line (p->p3_sx, p->p3_sy, p2->p3_sx, p2->p3_sy);
		}

		{
		g3s_point *p = bufptr[i];
		g3s_point *p2 = bufptr[0];
...
int g3_draw_sphere(g3s_point *pnt,fix rad)
{
	if (! (pnt->p3_codes & CC_BEHIND)) {
		...
		if (! (pnt->p3_codes & PF_OVERFLOW)) {
			...
			t = fixdivquadlong(fixmulaccum (r2,Canv_w2), pnt->p3_z);
