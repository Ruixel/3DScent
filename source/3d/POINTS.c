...
ITCM_CODE ubyte g3_code_point(g3s_point *p)
{
	ubyte cc=0;
...
void g3_project_point(g3s_point *p)
{
	...
	p->p3_sx = Canv_w2 + fixdivquadlong(fixmulaccum (p->p3_x,Canv_w2), p->p3_z);
	p->p3_sy = Canv_h2 - fixdivquadlong(fixmulaccum (p->p3_y,Canv_h2), p->p3_z);
...
//	q.low=q.high=0;
	q = fixmulaccum((pnt->x - View_position.x),View_matrix.fvec.x);
	q += fixmulaccum((pnt->y - View_position.y),View_matrix.fvec.y);
	q += fixmulaccum((pnt->z - View_position.z),View_matrix.fvec.z);
