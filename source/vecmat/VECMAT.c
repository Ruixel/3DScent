...
ITCM_CODE vms_vector *vm_vec_add(vms_vector *dest,vms_vector *src0,vms_vector *src1)
{
...
ITCM_CODE vms_vector *vm_vec_sub(vms_vector *dest,vms_vector *src0,vms_vector *src1)
{
	dest->x = src0->x - src1->x;
	dest->y = src0->y - src1->y;
	dest->z = src0->z - src1->z;
...
ITCM_CODE vms_vector *vm_vec_add2(vms_vector *dest,vms_vector *src)
{
	dest->x += src->x;
	dest->y += src->y;
	dest->z += src->z;
...
//dest can equal source
ITCM_CODE vms_vector *vm_vec_sub2(vms_vector *dest,vms_vector *src)
{
	dest->x -= src->x;
	dest->y -= src->y;
	dest->z -= src->z;
...
ITCM_CODE vms_vector *vm_vec_avg(vms_vector *dest,vms_vector *src0,vms_vector *src1)
{
...
/*
ITCM_CODE vms_vector *vm_vec_avg4(vms_vector *dest,vms_vector *src0,vms_vector *src1,vms_vector *src2,vms_vector *src3)
{
...
ITCM_CODE vms_vector *vm_vec_scale(vms_vector *dest,fix s)
{
...
ITCM_CODE vms_vector *vm_vec_copy_scale(vms_vector *dest,vms_vector *src,fix s)
{
...
//dest = src1 + k * src2
ITCM_CODE vms_vector *vm_vec_scale_add(vms_vector *dest,vms_vector *src1,vms_vector *src2,fix k)
{
...
//dest += k * src
ITCM_CODE vms_vector *vm_vec_scale_add2(vms_vector *dest,vms_vector *src,fix k)
{
...
//dest *= n/d
ITCM_CODE vms_vector *vm_vec_scale2(vms_vector *dest,fix n,fix d)
{
...
ITCM_CODE fix vm_vec_dotprod(vms_vector *v0,vms_vector *v1)
{
	quad q;

	q = fixmulaccum(v0->x,v1->x);
	q += fixmulaccum(v0->y,v1->y);
	q += fixmulaccum(v0->z,v1->z);

	return fixquadadjust(q);
}

ITCM_CODE fix vm_vec_dot3(fix x,fix y,fix z,vms_vector *v)
{
	quad q;
...
//#define quad_sqrt_i64(v) quad_sqrt ((unsigned long)(v), (unsigned long)((v) >> 32))

//returns magnitude of a vector
ITCM_CODE fix vm_vec_mag(vms_vector *v)
{
	quad q;

	q = fixmulaccum(v->x,v->x);
	q += fixmulaccum(v->y,v->y);
	q += fixmulaccum(v->z,v->z);

	return quad_sqrt(q);
}

//computes the distance between two points. (does sub and mag)
ITCM_CODE fix vm_vec_dist(vms_vector *v0,vms_vector *v1)
{
	vms_vector t;
...
ITCM_CODE fix vm_vec_mag_quick(vms_vector *v)
{
	fix a,b,c,bc;
...
//uses dist = largest + next_largest*3/8 + smallest*3/16
ITCM_CODE fix vm_vec_dist_quick(vms_vector *v0,vms_vector *v1)
{
	vms_vector t;
...
ITCM_CODE fix vm_vec_copy_normalize(vms_vector *dest,vms_vector *src)
{
	fix m;
...
ITCM_CODE fix vm_vec_copy_normalize_quick(vms_vector *dest,vms_vector *src)
{
	fix m;
...
ITCM_CODE fix vm_vec_normalized_dir_quick(vms_vector *dest,vms_vector *end,vms_vector *start)
{
...
//NOTE: the order of the parameters matches the vector subtraction
ITCM_CODE fix vm_vec_normalized_dir(vms_vector *dest,vms_vector *end,vms_vector *start)
{
	vm_vec_sub(dest,end,start);
...
ITCM_CODE vms_vector *vm_vec_normal(vms_vector *dest,vms_vector *p0,vms_vector *p1,vms_vector *p2)
{
...
ITCM_CODE void check_vec(vms_vector *v)
{
	fix check;
	int cnt = 0;
...
//your inputs are ok.
ITCM_CODE vms_vector *vm_vec_crossprod(vms_vector *dest,vms_vector *src0,vms_vector *src1)
{
	quad q;

	Assert(dest!=src0 && dest!=src1);

	q = fixmulaccum(src0->y,src1->z);
	q += fixmulaccum(-src0->z,src1->y);
	dest->x = fixquadadjust(q);

	q = fixmulaccum(src0->z,src1->x);
	q += fixmulaccum(-src0->x,src1->z);
	dest->y = fixquadadjust(q);

	q = fixmulaccum(src0->x,src1->y);
	q += fixmulaccum(-src0->y,src1->x);
...
//returns ptr to dest
//dest CANNOT equal either source
ITCM_CODE vms_vector *vm_vec_perp(vms_vector *dest,vms_vector *p0,vms_vector *p1,vms_vector *p2)
{
	vms_vector t0,t1;
...
//returned.
ITCM_CODE fixang vm_vec_delta_ang(vms_vector *v0,vms_vector *v1,vms_vector *fvec)
{
	vms_vector t0,t1;
...
ITCM_CODE fixang vm_vec_delta_ang_norm(vms_vector *v0,vms_vector *v1,vms_vector *fvec)
{
	fixang a;
...
ITCM_CODE vms_matrix *sincos_2_matrix(vms_matrix *m,fix sinp,fix cosp,fix sinb,fix cosb,fix sinh,fix cosh)
{
...
ITCM_CODE vms_matrix *vm_angles_2_matrix(vms_matrix *m,vms_angvec *a)
{
...
ITCM_CODE vms_matrix *vm_vec_ang_2_matrix(vms_matrix *m,vms_vector *v,fixang a)
{
...
ITCM_CODE vms_matrix *vm_vector_2_matrix(vms_matrix *m,vms_vector *fvec,vms_vector *uvec,vms_vector *rvec)
{
...
ITCM_CODE vms_vector *vm_vec_rotate(vms_vector *dest,vms_vector *src,vms_matrix *m)
{
	Assert(dest != src);
...
ITCM_CODE vms_matrix *vm_transpose_matrix(vms_matrix *m)
{
	fix t;
...
ITCM_CODE vms_matrix *vm_copy_transpose_matrix(vms_matrix *dest,vms_matrix *src)
{
	Assert(dest != src);
...
//dest CANNOT equal either source
ITCM_CODE vms_matrix *vm_matrix_x_matrix(vms_matrix *dest,vms_matrix *src0,vms_matrix *src1)
{
	Assert(dest!=src0 && dest!=src1);
...
/*
ITCM_CODE vms_matrix *vm_matrix_adjoint (vms_matrix *dest, vms_matrix *src)
{
	dest->rvec.x = fixmul(src->uvec.y, src->fvec.z) - fixmul(src->fvec.y,src->uvec.z);
	dest->uvec.x = -(fixmul(src->uvec.x,src->fvec.z) - fixmul(src->fvec.x,src->uvec.z));
	dest->fvec.x = fixmul(src->uvec.x,src->fvec.y) - fixmul(src->fvec.x,src->uvec.y);

	dest->rvec.y = -(fixmul(src->rvec.y,src->fvec.z) - fixmul(src->fvec.y,src->rvec.z));
	dest->uvec.y = fixmul(src->rvec.x,src->fvec.z) - fixmul(src->fvec.x,src->rvec.z);
	dest->fvec.y = -(fixmul(src->rvec.x,src->fvec.y) - fixmul(src->fvec.x,src->rvec.y));

	dest->rvec.z = fixmul(src->rvec.y,src->uvec.z) - fixmul(src->uvec.y,src->rvec.z);
	dest->uvec.z = -(fixmul(src->rvec.x,src->uvec.z)-fixmul(src->uvec.x,src->rvec.z));
	dest->fvec.z = fixmul(src->rvec.x,src->uvec.y)-fixmul(src->uvec.x,src->rvec.y);

	return dest;
}
*/
//extract angles from a matrix 
ITCM_CODE vms_angvec *vm_extract_angles_matrix(vms_angvec *a,vms_matrix *m)
{
	fix sinh,cosh,cosp;
...
ITCM_CODE vms_angvec *vm_extract_angles_vector_normalized(vms_angvec *a,vms_vector *v)
{
...
//extract heading and pitch from a vector, assuming bank==0
ITCM_CODE vms_angvec *vm_extract_angles_vector(vms_angvec *a,vms_vector *v)
{
	vms_vector t;
...
ITCM_CODE fix vm_dist_to_plane(vms_vector *checkp,vms_vector *norm,vms_vector *planep)
{
	vms_vector t;
