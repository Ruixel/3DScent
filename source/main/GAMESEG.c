...
ITCM_CODE void get_side_verts(short *vertlist,int segnum,int sidenum)
{
	int	i;
...
void pick_random_point_in_seg(vms_vector *new_pos, int segnum)
{
	...
	vnum = (random() * MAX_VERTICES_PER_SEGMENT) >> 15;
	vm_vec_sub(&vec2, &Vertices[Segments[segnum].verts[vnum]], new_pos);
	vm_vec_scale(&vec2, random());			//	rand() always in 0..1/2
