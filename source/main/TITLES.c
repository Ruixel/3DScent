...
//#include "texmap.h"
#include "mouse.h"
#include "ndsfunc.h"		// for bitblt_to_screen()
...
briefing_screen Briefing_screens[] = {
	...
	{	"brief02.pcx",   0,  2,  3,  20, 257, 177 },
	{	"brief03.pcx",   0,  3,  15,  22, 257, 177 },
	{	"brief02.pcx",   0,  4,  3,  20, 257, 177 },
...
//	if ((pcx_error=pcx_read_bitmap( filename, &title_bm, BM_LINEAR, New_pal ))!=PCX_ERROR_NONE)	{
	...
	if ((pcx_error=pcx_read_bitmap( filename, &grd_curcanv->cv_bitmap, BM_LINEAR, New_pal ))!=PCX_ERROR_NONE)	{
...
//	gr_bitmap( 0, 0, &title_bm );
	vertbuf[0].x = vertbuf[0].y = 0;
	vertbuf[2].x = i2f (grd_curcanv->cv_bitmap.bm_w);
	vertbuf[2].y = i2f (grd_curcanv->cv_bitmap.bm_h);
	scale_bitmap (&title_bm, vertbuf);
*/	bitblt_to_screen();
...
void show_bitmap_frame(void)
{
	...
	if (Bitmap_name[0] != 0) {
		...
		switch (Animating_bitmap_type) {
			case 0:	bitmap_canv = gr_create_sub_canvas(grd_curcanv, 191, 45, 64, 64);	break;
			case 1:	bitmap_canv = gr_create_sub_canvas(grd_curcanv, 161, 45, 94, 94);	break;	//	Adam: Change here for your new animating bitmap thing. 94, 94 are bitmap size.
...
void show_briefing_bitmap(grs_bitmap *bmp)
{
	...
	bitmap_canv = gr_create_sub_canvas(grd_curcanv, 220, 45, 117, 97);
	curcanv_save = grd_curcanv;
	grd_curcanv = bitmap_canv;
	gr_bitmapm(0, 0, bmp);
	bitblt_to_screen();
...
int show_briefing_message(int screen_num, char *message)
{
	...
	int	new_page=0;
	int blt_done;
	int Briefing_text_w, Briefing_text_h;

	if((bsp->text_ulx + bsp->text_width) > 251) {
		Briefing_text_w = 251;
	} else {
		Briefing_text_w = bsp->text_ulx + bsp->text_width;
	}
	if((bsp->text_uly + bsp->text_height) > 183) {
		Briefing_text_h = 183;
	} else {
		Briefing_text_h = bsp->text_uly + bsp->text_height;
	}
...
//				prev_ch = 10;							//	read to eoln
			...
			if (prev_ch != '\\') {
				...
				if (Briefing_text_y > Briefing_text_h) {
		...
		if ( key_check == KEY_ESC || key_check == KEY_B ) {
			rval = 1;
			done = 1;
		}
		...
		if ((key_check == KEY_SPACEBAR) || (key_check == KEY_ENTER) || (key_check == KEY_A))
			delay_count = 0;

		if (Briefing_text_x > Briefing_text_w) {
			Briefing_text_x = bsp->text_ulx;
			Briefing_text_y += 8;
		}

		if ((new_page) || (Briefing_text_y > Briefing_text_h)) {
			fix	start_time = 0;
			int	keypress;
...
int show_briefing_screen( int screen_num, int allow_keys)
{
	...
	if ((pcx_error=pcx_read_bitmap( Briefing_screens[screen_num].bs_name, &grd_curcanv->cv_bitmap, BM_LINEAR, New_pal ))!=PCX_ERROR_NONE)	{
