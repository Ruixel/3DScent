...
#define CURSOR_STRING		"_"
char NORMAL_CHECK_BOX[2];
char CHECKED_CHECK_BOX[2];
char NORMAL_RADIO_BOX[2];
char CHECKED_RADIO_BOX[2];
char SLIDER_LEFT[2];
char SLIDER_RIGHT[2];
char SLIDER_MIDDLE[2];
char SLIDER_MARKER[2];
...
//	bg.menu_canvas = gr_create_sub_canvas( &grd_curscreen->sc_canvas, x, y, w, h );
	bg.menu_canvas = gr_create_sub_canvas( grd_curcanv, x, y, w, h );
...
// draw a close box on the menu
	...
	if (show_sk)
	{
		...
		bg.background2 = gr_create_bitmap( grd_curcanv->cv_bitmap.bm_w, grd_curcanv->cv_bitmap.bm_h );
		Assert( bg.background2 != NULL );
		gr_bm_bitblt(grd_curcanv->cv_bitmap.bm_w, grd_curcanv->cv_bitmap.bm_h, 0, 0, 0, 0, &grd_curcanv->cv_bitmap, bg.background2 );
...
//			done=1;
	...
	{
		gr_set_current_canvas (&VR_screen_pages[1]);
		gr_bitmap(0, 0, bg.background2); 	
		gr_free_bitmap(bg.background2);
		gr_set_current_canvas( &VR_screen_pages[VR_current_page] );
		softkey_enable (0);
	}
...
int newmenu_get_filename( char * title, char * filespec, char * filename, int allow_abort_flag )
{
	...
	int close_x = 0, close_y = 0, close_size, close_box;
	dir_t	*dir;
...
//ReadFileNames:
	...
	dir = dir_find (filespec + 2);
	while ((find_filename = dir_findnext (dir))) {
		if (NumFiles<MAX_FILES)	{
			strncpy( &filenames[NumFiles*14], find_filename, 13 );
			if ( player_mode )	{
				char * p;
...
int newmenu_listbox1( char * title, int nitems, char * items[], int allow_abort_flag, int default_item, int (*listbox_callback)( int * citem, int *nitems, char * items[], int *keypress ), int close_box )
{
	...
	int close_x = 0, close_y = 0, close_size;
