...
network_endlevel_poll( int nitems, newmenu_item * menus, int * key, int citem )
{
	...
	for (i = 0; i < N_players; i++)
	{
		...
		if (Players[i].connected == 1)
		{
			...
			if (timer_get_fixed_seconds() > LastPacketTime[i]+ENDLEVEL_IDLE_TIME)
			{
...
newmenu:
	...
	for (i=0; i<N_players; i++) 
	{
		...
		LastPacketTime[i] = timer_get_fixed_seconds();
	}
...
void network_send_rejoin_sync(int player_num)
{
	...
	LastPacketTime[player_num] = timer_get_fixed_seconds();
...
*/	Netgame.players[N_players].connected = 1;
	...
	LastPacketTime[N_players] = timer_get_fixed_seconds();
	N_players++;
	Netgame.numplayers = N_players;
...
*/
	...
	LastPacketTime[playernum] = timer_get_fixed_seconds();
...
void network_sync_poll( int nitems, newmenu_item * menus, int * key, int citem )
{
	...
	if (!Network_rejoined && (timer_get_fixed_seconds() > t1+F1_0*2))
	{
		int i;
...
/*
	...
	if ( (Network_game_type == IPX_GAME) && (timer_get_fixed_seconds() > t1+F1_0*4) )
	{
		t1 = timer_get_fixed_seconds();
		network_send_game_list_request();
	} else if ( (Network_game_type == APPLETALK_GAME) && (timer_get_fixed_seconds() > t1+F1_0*10) )
	{
		t1 = timer_get_fixed_seconds();
		restart_net_searching(menus);
...
int zone_sort_func(const void *a, const void *b)
{
...
// still valid, since updates happen less frequently.
	...
	if (Network_game_type == APPLETALK_GAME) {
		...
		while ( (timer_get_fixed_seconds() < t1) && !Network_game_validated) // Wait 3 seconds for replies
			network_listen();
...
//			mprintf((0, "Send packet, %f secs, %d bytes.\n", f2fl(last_send_time), MySyncPack.data_size));
			...
			create_shortpos(&(ainfo.obj_info), &Objects[objnum]);
...
//	mprintf((0, "Gametime = %d, Frametime = %d.\n", GameTime, FrameTime));
	...
	LastPacketTime[TheirPlayernum] = timer_get_fixed_seconds();
...
//	mprintf((0, "Gametime = %d, Frametime = %d.\n", GameTime, FrameTime));
	...
	extract_shortpos( TheirObj, &(finfo.obj_info) );
