...
do_menu_again:
  ;
	if (!newmenu_get_filename( "Select Pilot\nPress X to delete", "*.plr", filename, allow_abort_flag ))	{
...
#ifdef SHAREWARE						// so when on registered, can start at next level
...
void InitPlayerPosition(int _random)
{
	int NewPlayer;
	...
	else if (_random == 1)
	{
		...
		do {
			...
			trys++;
			NewPlayer = random() % NumNetPlayerPositions;
