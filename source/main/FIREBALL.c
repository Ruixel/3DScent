...
object *object_create_debris(object *parent, int subobj_num)
{
	...
	obj->mtype.phys_info.velocity.x = RAND_MAX/2 - random();
	obj->mtype.phys_info.velocity.y = RAND_MAX/2 - random();
	obj->mtype.phys_info.velocity.z = RAND_MAX/2 - random();
	vm_vec_normalize_quick(&obj->mtype.phys_info.velocity);
	vm_vec_scale(&obj->mtype.phys_info.velocity,i2f(10 + (30 * random() / RAND_MAX)));
...
ITCM_CODE void draw_fireball(object *obj)
{
...
//			mprintf((0, "selected segment %i\n", seg_queue[tail]));
		...
		ind1 = (random() * MAX_SIDES_PER_SEGMENT) >> 15;
		ind2 = (random() * MAX_SIDES_PER_SEGMENT) >> 15;
		temp = side_rand[ind1];
...
int choose_drop_segment(void)
{
	int	pnum = 0;
	int	segnum = -1;
	int	initial_drop_depth = BASE_NET_DROP_DEPTH + ((random() * 10) >> 15);
	...
	while ((segnum == -1) && (cur_drop_depth > BASE_NET_DROP_DEPTH/2)) {
		...
		if (count == N_players) {
			...
			return (random() * Highest_segment_index) >> 15;
		}
	...
	switch (objp->contains_type) {
		case OBJ_POWERUP:
			for (count=0; count<objp->contains_count; count++) {
				...
				new_velocity.x += fixmul(old_mag+F1_0*32, random()*rand_scale - 16384*rand_scale);
				new_velocity.y += fixmul(old_mag+F1_0*32, random()*rand_scale - 16384*rand_scale);
				new_velocity.z += fixmul(old_mag+F1_0*32, random()*rand_scale - 16384*rand_scale);
...
//				mprintf(0, "                             pos = x=%d y=%d z=%d\n", obj->pos.x, obj->pos.y, obj->pos.z);
				...
				switch (obj->id) {
					...
					case POW_ENERGY:
						obj->lifeleft = (random() + F1_0*3) * 64;		//	Lives for 3 to 3.5 binary minutes (a binary minute is 64 seconds)
						if (Game_mode & GM_MULTI)
							obj->lifeleft /= 2;
						break;
					default:
...
//				else
				...
				new_velocity.x += (random()-16384)*2;
				new_velocity.y += (random()-16384)*2;
				new_velocity.z += (random()-16384)*2;
	...
	if (obj->lifeleft <= obj->ctype.expl_info.spawn_time) {
		...
		if ((del_obj->contains_count > 0) && !(Game_mode & GM_MULTI)) { // Multiplayer handled outside of this code!!
			...
			if (robptr->contains_count) {
				if (((random() * 16) >> 15) < robptr->contains_prob) {
					del_obj->contains_count = ((random() * robptr->contains_count) >> 15) + 1;
...
//			mprintf(0,"Creating %d new explosions\n",new_count-old_count);
			...
			for (e=old_count;e<new_count;e++) {
				...
				vm_vec_scale_add(&pos,v1,&vv0,random()*2);
				vm_vec_scale_add2(&pos,&vv1,random()*2);
