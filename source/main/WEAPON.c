...
byte Auto_primary_weapon_selection;
byte Auto_secondary_weapon_selection;
...
int pick_up_secondary(int weapon_index,int count)
{
	...
	if (Auto_secondary_weapon_selection && (Secondary_weapon<weapon_index) && (weapon_index != PROXIMITY_INDEX)) //  && (Players[Player_num].secondary_ammo[weapon_index] == count))
	...
	if (Auto_secondary_weapon_selection && (weapon_index != PROXIMITY_INDEX) && (Players[Player_num].secondary_ammo[Secondary_weapon] == 0))
		select_weapon(weapon_index,1, 0, 1);
...
int pick_up_primary(int weapon_index)
{
	...
	if (Auto_primary_weapon_selection && weapon_index>Primary_weapon)
...
int pick_up_ammo(int class_flag,int weapon_index,int ammo_count)
{
	...
	if (Auto_primary_weapon_selection &&
		Players[Player_num].primary_weapon_flags&(1<<weapon_index) && weapon_index>Primary_weapon && old_ammo==0)
		select_weapon(weapon_index,0,0,1);
