...
static size_t demo_buffer_size = 0;
static size_t demo_buffer_pos = 0;

static int load_demo_into_memory(FILE *fp)
{
	fseek(fp, 0, SEEK_END);
	demo_buffer_size = (size_t)ftell(fp);
	fseek(fp, 0, SEEK_SET);

	demo_buffer = (char *)malloc(demo_buffer_size);
	if (!demo_buffer) {
		printf("Failed to allocate %zu bytes for demo buffer\n", demo_buffer_size);
		demo_buffer_size = 0;
		return 0;
	}

	size_t read = fread(demo_buffer, 1, demo_buffer_size, fp);
	if (read != demo_buffer_size) {
		printf("Failed to read full demo: got %zu of %zu\n", read, demo_buffer_size);
		free(demo_buffer);
		demo_buffer = NULL;
		demo_buffer_size = 0;
		return 0;
...
static void unload_demo_buffer(void)
{
	if (demo_buffer) {
		free(demo_buffer);
		demo_buffer = NULL;
...
int newdemo_get_percent_done()	{
	if ( Newdemo_state == ND_STATE_PLAYBACK )	{
		if (demo_buffer_size == 0) return 0;
		return ((int)demo_buffer_pos * 100) / (int)demo_buffer_size;
	}
...
{
	size_t total = (size_t)elsize * (size_t)nelem;
	if (demo_buffer_pos + total > demo_buffer_size) {
		size_t avail = (demo_buffer_pos < demo_buffer_size) ? (demo_buffer_size - demo_buffer_pos) : 0;
		size_t full_elems = (elsize > 0) ? (avail / (size_t)elsize) : 0;
		if (full_elems > 0) {
			memcpy(buffer, demo_buffer + demo_buffer_pos, full_elems * (size_t)elsize);
			demo_buffer_pos += full_elems * (size_t)elsize;
		}
		nd_bad_read = -1;
		return (int)full_elems;
	}
	memcpy(buffer, demo_buffer + demo_buffer_pos, total);
	demo_buffer_pos += total;
	return nelem;
}
...
void newdemo_record_multi_score(int pnum, int score)
{
	...
	nd_write_byte((ubyte)pnum);
	nd_write_int(score - Players[pnum].score);
	start_time();
}
...
int newdemo_read_demo_start(int rnd_demo)
{
...
#ifdef SHAREWARE
	{
		...
		if (demo_buffer_pos < demo_buffer_size) {
			c = (unsigned char)demo_buffer[demo_buffer_pos];
			demo_buffer_pos++;
		} else {
			c = 0;
			nd_bad_read = -1;
...
{
	int done, segnum, side, objnum, soundno, angle, volume, i, time;
	object *obj;
	byte c;
	...
	while( !done )	{
		...
		case ND_EVENT_REARVIEW:
			if ((Newdemo_vcr_state == ND_STATE_PLAYBACK) || (Newdemo_vcr_state == ND_STATE_FASTFORWARD) || (Newdemo_vcr_state == ND_STATE_ONEFRAMEFORWARD)) {
				...
				if (saved_rearview_cockpit == CM_REAR_VIEW)
...
void newdemo_goto_end()
{
	...
	demo_buffer_pos = (demo_buffer_size >= 2) ? (demo_buffer_size - 2) : 0;
	nd_read_byte(&level);
	...
	if (Newdemo_game_mode & GM_MULTI) {
		demo_buffer_pos = (demo_buffer_size >= 10) ? (demo_buffer_size - 10) : 0;
	...
	demo_buffer_pos = (demo_buffer_size >= 12) ? (demo_buffer_size - 12) : 0;
	nd_read_short(&frame_length);
	if ((size_t)frame_length <= demo_buffer_pos)
		demo_buffer_pos -= (size_t)frame_length;
	else
		demo_buffer_pos = 0;
	nd_read_int(&NewdemoFrameCount);
	NewdemoFrameCount--;
	demo_buffer_pos += 4;
	if (demo_buffer_pos > demo_buffer_size) demo_buffer_pos = demo_buffer_size;
	newdemo_read_frame_information();
	Newdemo_vcr_state = ND_STATE_PAUSED;
	return;
}
#else
void newdemo_goto_end()
{
	...
	demo_buffer_pos = (demo_buffer_size >= 4) ? (demo_buffer_size - 4) : 0;
	nd_read_short(&byte_count);
	if ((size_t)(2 + byte_count) <= demo_buffer_pos)
		demo_buffer_pos -= (size_t)(2 + byte_count);
	else
		demo_buffer_pos = 0;

	nd_read_short(&frame_length);
	loc = (int)demo_buffer_pos;
	if (Newdemo_game_mode & GM_MULTI)
		nd_read_byte(&Newdemo_players_cloaked);
	else
		nd_read_byte(&bbyte);
...
void newdemo_back_frames(int frames)
{
	...
	for (i = 0; i < frames; i++)
	{
		if (demo_buffer_pos >= 10) demo_buffer_pos -= 10; else demo_buffer_pos = 0;
		nd_read_short(&last_frame_length);
		{
			long delta = (long)8 - (long)last_frame_length;
			if (delta >= 0)
				demo_buffer_pos += (size_t)delta;
			else {
				size_t back = (size_t)(-delta);
				demo_buffer_pos = (demo_buffer_pos >= back) ? (demo_buffer_pos - back) : 0;
			}
		}
...
void newdemo_playback_one_frame()
{
	...
	if ((Newdemo_vcr_state == ND_STATE_REWINDING) || (Newdemo_vcr_state == ND_STATE_ONEFRAMEBACKWARD))
	{
		...
		if (NewdemoFrameCount <= 0)
			nd_playback_total = nd_recorded_total;
		else
		...
		if ((playback_style == NORMAL_PLAYBACK) && (NewdemoFrameCount > 10))
			if ((nd_playback_total * INTERPOL_FACTOR) < nd_recorded_total) {
				...
				nd_playback_total = nd_recorded_total + FrameTime;
...
void newdemo_start_playback(char * filename)
{
	...
	if (!load_demo_into_memory(infile)) {
		fclose(infile);
		infile = NULL;
		return;
	}
	...
	Newdemo_size = (int)demo_buffer_size;
	nd_bad_read = 0;
	Newdemo_at_eof = 0;
	NewdemoFrameCount = 0;
