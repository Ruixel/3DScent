...
{
	vms_vector *pointlist[3];

	pointlist[0] = &p0->p3_vec;
	pointlist[1] = &p1->p3_vec;
	pointlist[2] = &p3->p3_vec;
...
//	g3_check_and_draw_tmap(3,pointlist,uvl_list1,terrain_bm,NULL,NULL);
	g3_draw_tmap_tex(3, pointlist, uvl_list1, terrain_bm);
/*	if (terrain_outline) {
...
*/
	pointlist[0] = &p1->p3_vec;
	pointlist[1] = &p2->p3_vec;
...
//	g3_check_and_draw_tmap(3,pointlist,uvl_list2,terrain_bm,NULL,NULL);
	g3_draw_tmap_tex(3, pointlist, uvl_list2, terrain_bm);
/*	if (terrain_outline) {
		int lsave=Lighting_on;
		Lighting_on=0;
		gr_setcolor(BM_XRGB(31,0,0));
...
//		atexit(free_light_table);		//first time
	...
	light_array = malloc (grid_w * grid_h);
