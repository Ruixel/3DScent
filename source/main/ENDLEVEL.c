...
void do_endlevel_frame()
{
	...
	if (!outside_mine) {
		...
		if ((explosion_wait1-=FrameTime) < 0) {
			...
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.rvec,(random()-RAND_MAX/2)*15);
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.uvec,(random()-RAND_MAX/2)*15);
			...
			if (segnum != -1) {
				...
				if (random()<10000 || ++sound_count==7) {		//pseudo-random
					digi_link_sound_to_pos( SOUND_TUNNEL_EXPLOSION, segnum, 0, &tpnt, 0, F1_0 );
					sound_count=0;
				}
			}

			explosion_wait1 = 0x2000 + random()/4;
	...
	if (Endlevel_sequence >= EL_FLYTHROUGH && Endlevel_sequence < EL_OUTSIDE)
		if ((explosion_wait2-=FrameTime) < 0) {
			...
			vm_vec_copy_scale(&tpnt,&ConsoleObject->orient.rvec,(random()-RAND_MAX/2)*100);
			vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.uvec,(random()-RAND_MAX/2)*100);
			vm_vec_add2(&tpnt,&ConsoleObject->pos);

			if (Endlevel_sequence == EL_FLYTHROUGH)
				vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.fvec,random()*200);
			else
				vm_vec_scale_add2(&tpnt,&ConsoleObject->orient.fvec,random()*60);
			...
			if (hit_data.hit_type==HIT_WALL && hit_data.hit_seg!=-1)
				object_create_explosion(hit_data.hit_seg,&hit_data.hit_pnt,i2f(3)+random()*6,VCLIP_SMALL_EXPLOSION);

			explosion_wait2 = (0xa00 + random()/8)/2;
		}
...
//				Interpolation_method = 0;
				...
				g3_draw_rod_tmap(satellite_bitmap,&p.p3_vec,SATELLITE_WIDTH,&top_pnt.p3_vec,SATELLITE_WIDTH,f1_0);
...
void generate_starfield()
{
	...
	for (i=0;i<MAX_STARS;i++) {
		...
		stars[i].x = (random() - RAND_MAX/2) << 14;
		stars[i].z = (random() - RAND_MAX/2) << 14;
		stars[i].y = (random()/2) << 14;
...
try_again:
	...
	while (cfgets(line,LINE_LEN,ifile)) {
		...
		for (p=line+len;p>line && isspace(*p);*p--=0);
		...
		switch (var) {
			...
			case 4: {						//planet bitmap
				...
				satellite_bitmap ->key = 0xffff0002;
