...
#define SAVE_FILE_ID			(('D' << 24) | ('P' << 16) | ('L' << 8) | ('R' << 0))
...
extern byte Auto_primary_weapon_selection;
extern byte Auto_secondary_weapon_selection;
...
//	m[4].type = NM_TYPE_MENU; m[4].text = CONTROL_TEXT(5);
	...
	control_choice = newmenu_do1( NULL, TXT_CHOOSE_INPUT, 2, m, NULL, control_choice );
...
int read_player_file()
{
	...
	FILE *file;
	save_info info;
	int errno_ret = 0, rewrite_player_file = 0;

	if (!(Player_num>=0 && Player_num<MAX_PLAYERS))
        Error("Error loading player profiles");
...
//			}
	...
	if (fread (&Auto_primary_weapon_selection, sizeof (byte), 1, file) != 1)
	{
		Auto_primary_weapon_selection = 1;
		Auto_secondary_weapon_selection = 1;
		rewrite_player_file = 1;
	}
	else
	{
		fread (&Auto_secondary_weapon_selection, sizeof (byte), 1, file);
	}
	...
	if (rewrite_player_file)
		write_player_file();

	sprintf(filename,"%s.csk",Players[Player_num].callsign);
	softkey_read (filename);
...
//	}
	...
	fwrite (&Auto_secondary_weapon_selection, sizeof (byte), 1, file);
