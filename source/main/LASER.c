...
ITCM_CODE void Laser_render(object *obj)
{
...
//	obj->lifeleft = Weapon_info[obj->id].lifetime;
	...
	if ((obj->type == OBJ_WEAPON) && (obj->id == FLARE_ID))
		obj->lifeleft += (random()-16384) << 2;		//	add in -2..2 seconds
...
int do_laser_firing(int objnum, int weapon_num, int level, int flags, int nfires)
{
	...
	Laser_offset = ((F1_0*2)*(random()%10))/10;
	...
	switch (weapon_num) {
		...
		case VULCAN_INDEX: {
			...
			//if (rand() > 24576)
			//	make_sound = 1;
			Laser_player_fire_spread( objp, VULCAN_ID, 6, random()/8 - 32767/16, random()/8 - 32767/16, make_sound, 0);
			if (nfires > 1) {
				Laser_player_fire_spread( objp, VULCAN_ID, 6, random()/8 - 32767/16, random()/8 - 32767/16, 0, 0);
				if (nfires > 2) {
					Laser_player_fire_spread( objp, VULCAN_ID, 6, random()/8 - 32767/16, random()/8 - 32767/16, 0, 0);
				}
			}
...
//			mprintf((0, "Fusion multiplier %f.\n", f2fl(Fusion_charge)));
			...
			force_vec.x = (force_vec.x >> 4) + random() - 16384;
			force_vec.y = (force_vec.y >> 4) + random() - 16384;
			force_vec.z = (force_vec.z >> 4) + random() - 16384;
	...
	if (objp->id == SMART_ID) {
		...
		if (numobjs == 0) {
			...
			for (i=0; i<NUM_SMART_CHILDREN; i++) {
				if (parent_type == OBJ_PLAYER) {
					int	hobjnum;
					hobjnum = create_homing_missile(objp, objlist[(random() * numobjs) >> 15].objnum, PLAYER_SMART_HOMING_ID, make_sound);
					// mprintf((0, "Object #%i is a PLAYER smart blob.\n", hobjnum));
				} else {
					int	hobjnum;
					hobjnum = create_homing_missile(objp, objlist[(random() * numobjs) >> 15].objnum, ROBOT_SMART_HOMING_ID, make_sound);
					// mprintf((0, "Object #%i is a robot smart blob.\n", hobjnum));
				}
				make_sound = 0;
			}
