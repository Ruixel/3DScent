...
#define WALL_FUELCEN_COLOR				BM_XRGB( 29, 27, 13 )
#define WALL_CONTROLCEN_COLOR			BM_XRGB( 29, 0, 0 )
#define WALL_ROBOTMAKER_COLOR			BM_XRGB( 29, 0, 31 )
#define WALL_PLAYER_START_COLOR			BM_XRGB( 31, 0, 31 )
#define WALL_UNVISITED_COLOR			BM_XRGB( 0, 0, 25 )

ubyte	wall_normal_color;
ubyte	wall_door_color;
ubyte	wall_door_blue;
ubyte	wall_door_gold;
ubyte	wall_door_red;
ubyte	wall_fuelcen_color;
ubyte	wall_controlcen_color;
ubyte	wall_robotmaker_color;
ubyte	wall_player_start_color;
ubyte	wall_unvisited_color;
...
//	gr_bm_ubitblt( Page.cv_bitmap.bm_w, Page.cv_bitmap.bm_h, Page.cv_bitmap.bm_x, Page.cv_bitmap.bm_y, 0, 0, &Page.cv_bitmap, &VR_screen_pages.cv_bitmap );
}
...
void do_automap_multiscreen (int x, int y, int w, int h)
{
	...
	gr_init_sub_canvas(&DrawingPage,grd_curcanv, x, y, w, h);
...
//	ViewMatrix = Objects[Players[Player_num].objnum].orient;
	...
	adjust_segment_limit(set_segment_depths(Objects[Players[Player_num].objnum].segnum, Automap_visited));
...
//	create_name_canv();
	...
	gr_palette_clear();
//	gr_init_sub_canvas(&Page,&VR_screen_pages[0],0, 0, 256, 192);
//	gr_init_sub_canvas(&DrawingPage,&Page,38,77,180,93);
//	gr_init_sub_canvas(&DrawingPage,&VR_screen_pages[0],0, 0, 256, 192);
...
*/
		...
		gr_clear_canvas (0);
		//memset (DrawingPage.cv_bitmap.bm_data, 0, DrawingPage.cv_bitmap.bm_w * DrawingPage.cv_bitmap.bm_h);
		draw_automap();
		bitblt_to_screen ();
...
//				DrawingListBrightZ[nbright] = (short)Segment_points[Edges[e-Edges].verts[0]].p3_z;
				nbright++;
...
void add_segment_edges(segment *seg)
{
	...
	for (sn=0;sn<MAX_SIDES_PER_SEGMENT;sn++) {
		...
		case SEGMENT_IS_FUELCEN:
			color = wall_fuelcen_color;
			break;
		case SEGMENT_IS_CONTROLCEN:
			color = wall_controlcen_color;
			break;
		case SEGMENT_IS_ROBOTMAKER:
			color = wall_robotmaker_color;
			break;
		}
...
void automap_build_edge_list()
{
	...
	wall_normal_color = WALL_NORMAL_COLOR;
	wall_door_color = WALL_DOOR_COLOR;
	wall_door_blue = WALL_DOOR_BLUE;
	wall_door_gold = WALL_DOOR_GOLD;
	wall_door_red = WALL_DOOR_RED;
	wall_fuelcen_color = WALL_FUELCEN_COLOR;
	wall_controlcen_color = WALL_CONTROLCEN_COLOR;
	wall_robotmaker_color = WALL_ROBOTMAKER_COLOR;
	wall_player_start_color = WALL_PLAYER_START_COLOR;
	wall_unvisited_color = WALL_UNVISITED_COLOR;
