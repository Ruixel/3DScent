...
{
	if ((Cockpit_mode == CM_STATUS_BAR) && (Last_msg_ycrd != -1) && (VR_offscreen_buffer.cv_bitmap.bm_y >= 6)) {
...
void HUD_render_message_frame()
{
	...
	if (HUD_nmessages > 0 )	{
		...
		if ((Cockpit_mode == CM_STATUS_BAR) && (VR_offscreen_buffer.cv_bitmap.bm_y >= 19)) {
...
#else
	if (Player_exploded) { //(ConsoleObject->flags & OF_EXPLODING)) {
#endif
		gr_set_curfont( GAME_FONT );
		if (HUD_color == -1)
			HUD_color = BM_XRGB(0,28,0);
