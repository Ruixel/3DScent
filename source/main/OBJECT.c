...
ITCM_CODE void draw_object_blob(object *obj,bitmap_index bmi)
{
...
ITCM_CODE void draw_object_tmap_rod(object *obj,bitmap_index bitmapi,int lighted)
{
	...
	g3_draw_rod_tmap(bitmap,&bot_p.p3_vec,obj->size,&top_p.p3_vec,obj->size,light);
}
...
ITCM_CODE void draw_cloaked_object(object *obj,fix light,fix *glow,fix cloak_start_time,fix cloak_end_time, ushort *alt_tmap)
{
...
ITCM_CODE void draw_polygon_object(object *obj)
{
	fix light;
//	int	imsave;
...
//		Interpolation_method = 1;
	...
	if (obj->movement_type == MT_PHYSICS) {
		...
		if (obj->mtype.phys_info.flags & PF_USES_THRUST && obj->type==OBJ_PLAYER && obj->id==Player_num) {
			...
			engine_glow_value += fixmul (fixdiv(thrust_mag,Player_ship->max_thrust), 52428); // *4/5
		}
		else {
			fix speed = vm_vec_mag_quick(&obj->mtype.phys_info.velocity);
			engine_glow_value += fixmul (fixdiv(speed,MAX_VELOCITY), 52428); // *4/5
		}
	}
...
ITCM_CODE void set_robot_location_info(object *objp)
{
...
void create_small_fireball_on_object(object *objp, fix size_scale, int sound_flag)
{
	...
	size = fixmul(size_scale, F1_0 + random()*4);
...
ITCM_CODE void render_object(object *obj)
{
	int	mld_save;
