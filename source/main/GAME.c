...
grs_canvas  VR_offscreen_buffer;		// The offscreen data buffer
...
int Cockpit_mode=CM_FULL_SCREEN;		//set game.h for values
...
//mwa	}
	...
	case CM_FULL_SCREEN:
		VR_offscreen_buffer->cv_bitmap.bm_x = 0;
		VR_offscreen_buffer->cv_bitmap.bm_y = 0;
		VR_offscreen_buffer->cv_bitmap.bm_w = VR_render_width;
		VR_offscreen_buffer->cv_bitmap.bm_h = VR_render_height;
	...
	case CM_LETTERBOX:	{
		...
		VR_offscreen_buffer->cv_bitmap.bm_x = x;
		VR_offscreen_buffer->cv_bitmap.bm_y = y;
		VR_offscreen_buffer->cv_bitmap.bm_w = w;
		VR_offscreen_buffer->cv_bitmap.bm_h = h;
...
//	if (!VR_offscreen_buffer)	{
		...
		gr_init_sub_canvas( &VR_offscreen_buffer, grd_curcanv, 0, 0, render_w, render_h );
		gr_init_sub_canvas( &VR_screen_pages[0], &VR_offscreen_buffer, 0, 0, 256, 192 );
		gr_init_sub_canvas( &VR_screen_pages[1], &VR_offscreen_buffer, 0, 0, 256, 192 );
		VR_screen_pages[1].cv_bitmap.bm_data += 256 * 192;
		gr_set_current_canvas (&VR_screen_pages[0]);
//		VR_offscreen_buffer;
		//gr_init_sub_canvas( VR_offscreen_buffer, &VR_screen_pages, 0, 0, render_w, render_h );
...
ITCM_CODE void game_render_frame_mono(void)
{
...
*///				gr_bm_ubitblt( VR_offscreen_buffer->cv_w, VR_offscreen_buffer->cv_h, VR_offscreen_buffer->cv_bitmap.bm_x, VR_offscreen_buffer->cv_bitmap.bm_y, 0, 0, &VR_offscreen_buffer->cv_bitmap, &VR_screen_pages.cv_bitmap );
//			}
		} else	{
	//		gr_ibitblt( &VR_offscreen_buffer->cv_bitmap, &VR_screen_pages.cv_bitmap, 0 );
		}
	}
#endif
#if 0
...
void diminish_palette_towards_normal(void)
{
	...
	if (FrameTime < F1_0/DIMINISH_RATE) {
		if (random() < FrameTime*DIMINISH_RATE/2)	//	Note: rand() is in 0..32767, and 8 Hz means decrement every frame
			dec_amount = 1;
	} else {
		...
		{ // Note the link to above!
					...
					if ((Players[Player_num].energy < F1_0*2) && (Auto_fire_fusion_cannon_time == 0)) {
						...
						if (Fusion_next_sound_time < GameTime) {
							if (Fusion_charge > F1_0*2) {
								...
								apply_damage_to_player(ConsoleObject, ConsoleObject, random() * 4);
							} else {
							...
							Fusion_next_sound_time = GameTime + F1_0/8 + random()/4;
						}
					}
				}
			}
			...
			if (Auto_fire_fusion_cannon_time) {
				if (Primary_weapon != FUSION_INDEX)
					...
					ConsoleObject->mtype.phys_info.rotvel.x += (random() - 16384)/8;
					ConsoleObject->mtype.phys_info.rotvel.z += (random() - 16384)/8;
...
int mark_player_path_to_segment(int segnum)
{
	...
	for (i=1; i<player_path_length; i++) {
		...
		obj->lifeleft = F1_0*100 + random() * 4;
	}
