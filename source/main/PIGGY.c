...
#ifdef SHAREWARE
#define PIGGY_BUFFER_SIZE		(1024*1024)
...
//	hashtable_init( &AllDigiSndNames, MAX_SOUND_FILES );
	...
	if (GameBitmaps == NULL) {
		GameBitmaps = (grs_bitmap *)malloc(sizeof(grs_bitmap) * MAX_BITMAP_FILES);
		if (GameBitmaps == NULL)
			Error("Cannot allocate space for game bitmaps in piggy.c");
	}

	if (AllBitmaps == NULL) {
		AllBitmaps = (BitmapFile *)malloc(sizeof(BitmapFile) * MAX_BITMAP_FILES);
		if (AllBitmaps == NULL)
			Error("Cannot allocate space for bitmap filenames in piggy.c");
	}
	
/*	if (AllSounds == NULL) {
		AllSounds = (SoundFile *)malloc(sizeof(SoundFile) * MAX_SOUND_FILES);
		if (AllSounds == NULL)
...
//		temp_bitmap.bm_flags = BM_FLAG_PAGED_OUT;
		...
		temp_bitmap.bm_data = 0;//Piggy_bitmap_cache_data;
		temp_bitmap.key = -1;
		temp_bitmap.texname = -1;
		...
		temp_bitmap.bm_flags = BM_FLAG_PAGED_OUT | GameBitmapFlags[i+1];
...
//		cfread( &sndh, sizeof(DiskSoundHeader), 1, Piggy_fp );
		...
		sndh.length = read_int(Piggy_fp);
		sndh.data_length = read_int(Piggy_fp);
		sndh.offset = read_int(Piggy_fp);

		//size -= sizeof(DiskSoundHeader);
		GameSounds[Num_sound_files].length = sndh.length;
//		GameSounds[Num_sound_files].data = (ubyte *)(sndh.offset + header_size + (sizeof(int)*2)+Pigdata_start);
		strncpy (Sounds_name[Num_sound_files], sndh.name, 8);
...
void adpcm_coder(char *indata, ubyte *outdata, int len);
...
void piggy_bitmap_set_flag( bitmap_index bitmap )
{
	...
	bmp->bm_flags = GameBitmapFlags[i] | BM_FLAG_PAGED_OUT;
}
...
//	ubyte *src;
	...
	if ( bmp->bm_flags & BM_FLAG_PAGED_OUT && !(bmp->bm_flags & BM_FLAG_IN_VRAM))	{
		stop_time();
	ReDoIt:
...
*/
			bmp->bm_data = malloc (zsize + 4);
			memcpy( bmp->bm_data, &zsize, sizeof(int) );
//			Piggy_bitmap_cache_next += sizeof(int);
			descent_critical_error = 0;
			temp = cfread( bmp->bm_data + 4, 1, zsize-4, Piggy_fp );
			if ( descent_critical_error )	{
				piggy_critical_error();
				goto ReDoIt;
			}
//			Piggy_bitmap_cache_next += zsize-4;
			...
			bmp->bm_data = malloc (bmp->bm_h*bmp->bm_w);
			temp = cfread( bmp->bm_data, 1, bmp->bm_h*bmp->bm_w, Piggy_fp );
			if ( descent_critical_error )	{
				piggy_critical_error();
				goto ReDoIt;
			}
...
void piggy_bitmap_page_out_all()
{
	...
	for (i=0; i<Num_bitmap_files; i++ )		{
		if ( GameBitmapOffset[i] > 0 )	{	// Don't page out bitmaps read from disk!!!
			GameBitmaps[i].bm_flags = BM_FLAG_PAGED_OUT | GameBitmapFlags[i];
			if (GameBitmaps[i].bm_data)
				free (GameBitmaps[i].bm_data);
			GameBitmaps[i].bm_data = 0;//Piggy_bitmap_cache_data;
			GameBitmaps[i].texname = -1;
			GameBitmaps[i].key = -1;
		}
	}
