...
multi_do_message(unsigned char *buf)
{
	...
	if (((colon = strrchr((char *)buf+loc, ':')) == NULL) || ((int)colon-((int)buf+loc) < 1) || ((int)colon-((int)buf+loc) > CALLSIGN_LEN))
	{
		digi_play_sample(SOUND_HUD_MESSAGE, F1_0);
	...
	{
		if ( (!strnicmp(Players[Player_num].callsign, (char *)buf+loc, (int)colon-((int)buf+loc))) ||
			  ((Game_mode & GM_TEAM) && ( (get_team(Player_num) == atoi((char *)buf+loc)-1) || !strnicmp(Netgame.team_name[get_team(Player_num)], (char *)buf+loc, (int)colon-((int)buf+loc)))) )
		{
			digi_play_sample(SOUND_HUD_MESSAGE, F1_0);	
...
multi_send_destroy_controlcen(int objnum, int player)
{
	...
	*(ushort *)(multibuf+1) = (short)objnum;
	multibuf[3] = player;
...
multi_send_reappear()
{
	...
	*(short *)(multibuf+1) = (short)Players[Player_num].objnum;
...
multi_send_hostage_door_status(int wallnum)
{
	...
	*(fix *)(multibuf+count) = (int)Walls[wallnum].hps;					count += 4;
