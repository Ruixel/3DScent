...
void do_option ( int select) 
{
	switch (select) {
		...
		case MENU_DEMO_PLAY:
			{ 
				char demo_file[16];
				if (newmenu_get_filename( "Select Demo\nPress X to delete", "*.dem", demo_file, 1 ))	{
...
byte	Max_perspective_depths[NUM_DETAIL_LEVELS-1] =					{ 1,  2,  3,  5,  8};//pc
//byte	Max_perspective_depths[NUM_DETAIL_LEVELS-1] =					{ 1,  2,  3,  4,  5};//mac
...
void sound_menuset( int nitems, newmenu_item *items, int *last_key, int citem )
{
	if ( Config_digi_volume != items[0].value )	{
		...
		digi_set_digi_volume( items[0].value );
...
int do_ingame_menu()
{
	...
	do {
		...
		switch(i)	{
			case 0: do_options_menu();			break;
			case 2: state_save_all(0);			break;
			case 3: state_restore_all(1);		break;
			case 5:	return 0;
		}
	} while( i>-1 );
	return 1;
...
extern byte Auto_primary_weapon_selection, Auto_secondary_weapon_selection;
