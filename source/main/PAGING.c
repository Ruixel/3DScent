	...
	for (i=0;i<Num_effects;i++)	{
		if ( Effects[i].changing_wall_texture == tmap_num )	{
			...
			if (Effects[i].dest_bm_num > -1)
				PIGGY_SET_FLAG( Textures[Effects[i].dest_bm_num] );	//use this bitmap when monitor destroyed
...
void paging_touch_model( int modelnum )
{
	...
	for (i=0;i<pm->n_textures;i++)	{
		PIGGY_SET_FLAG( ObjBitmaps[ObjBitmapPtrs[pm->first_texture+i]] );
...
void paging_touch_weapon( int weapon_type )
{
	...
	if ( Weapon_info[weapon_type].picture.index )	{
		PIGGY_SET_FLAG( Weapon_info[weapon_type].picture );
	}		
	...
	case WEAPON_RENDER_BLOB:
		PIGGY_SET_FLAG( Weapon_info[weapon_type].bitmap );
		break;
	}
}
	...
	switch (obj->render_type) {
		...
		case RT_POLYOBJ:
			if ( obj->rtype.pobj_info.tmap_override != -1 ) {
				PIGGY_SET_FLAG( Textures[obj->rtype.pobj_info.tmap_override] );
			} else {
...
//		paging_draw_orb();
		if ( Walls[i].clip_num > -1 )	{
			...
			for (j=0; j < anim->num_frames; j++ )	{
				PIGGY_SET_FLAG( Textures[anim->frames[j]] );
			}
		}
	}
}
