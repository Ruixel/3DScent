...
static grs_bitmap	texmerge_in_vram;
...
//	}
	...
	if (bitmap_top->bm_flags & BM_FLAG_PAGED_OUT)
		bitmap_top->bm_flags &= ~BM_FLAG_IN_VRAM;
	if (bitmap_bottom->bm_flags & BM_FLAG_PAGED_OUT)
		bitmap_bottom->bm_flags &= ~BM_FLAG_IN_VRAM;
	...
	if (bitmap_top->bm_flags & BM_FLAG_SUPER_TRANSPARENT)	{
		...
		Cache[least_recently_used].bitmap->bm_flags = BM_FLAG_TRANSPARENT | BM_FLAG_NO_PAGE_OUT;
		...
		Cache[least_recently_used].bitmap->bm_flags = (bitmap_bottom->bm_flags & (~BM_FLAG_RLE)) | BM_FLAG_NO_PAGE_OUT;
		Cache[least_recently_used].bitmap->avg_color = bitmap_bottom->avg_color;
	}

	bitmap_top->bm_flags |= BM_FLAG_NO_PAGE_OUT;
	bitmap_bottom->bm_flags |= BM_FLAG_NO_PAGE_OUT;
	...
	Cache[least_recently_used].bitmap->key = (orient << 30) | (Textures[tmap_top&0x3FFF].index << 16) | Textures[tmap_bottom].index;
