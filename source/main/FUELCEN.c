   ...
   //	  <<<<<<<<<<<<<<<< Num robots in mine >>>>>>>>>>>>>>>>>>>>>>>>>>    <<<<<<<<<<<< Max robots in mine >>>>>>>>>>>>>>>
	...
	case 0:		// Wait until next robot can generate
		...
		else 
		{
			...
			top_time = dist_to_player/64 + random() * 2 + F1_0*2;
			if ( top_time > ROBOT_GEN_TIME )
				top_time = ROBOT_GEN_TIME + random();
			if ( top_time < F1_0*2 )
				top_time = F1_0*3/2 + random()*2;
		}
		...
		if (robotcen->Timer > (Vclip[VCLIP_MORPHING_ROBOT].play_time/2) )	{
			...
			if (RobotCenters[matcen_num].robot_flags != 0) {
				...
				if (num_types == 1)
					...
					type = legal_types[(random() * num_types) / 32768];
...
//	mprintf( (0, "CCT: %.1f\n", f2fl(controlcen->Timer)));
	...
	if (fc > 16)
		fc = 16;
	ConsoleObject->mtype.phys_info.rotvel.x += fixmul(random() - 16384, 3*F1_0/16 + (F1_0*(16-fc))/32);
	ConsoleObject->mtype.phys_info.rotvel.z += fixmul(random() - 16384, 3*F1_0/16 + (F1_0*(16-fc))/32);
