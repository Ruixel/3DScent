...
//int	Clear_window=0;			//	1 = Clear whole background window, 2 = clear view portals into rest of world, 0 = no clear
...
#define Face_reflectivity (F1_0/2);
...
void draw_3d_reticle(fix eye_offset)
{
	...
	vm_vec_scale_add( &v2, &v1, &Viewer->orient.rvec, -F1_0/2 );
	vm_vec_scale_add2( &v2, &Viewer->orient.uvec, F1_0/2 );
	...
	if ( reticle_canvas == NULL )	{
		...
		reticle_canvas->cv_bitmap.bm_flags = BM_FLAG_TRANSPARENT | BM_FLAG_NO_PAGE_OUT | BM_FLAG_FORCE_REFRESH;
	}
	...
	g3_draw_tmap_func(4,pointlist,uvl,&reticle_canvas->cv_bitmap);
...
ITCM_CODE void render_face(int segnum, int sidenum, int nv, short *vp, int tmap1, int tmap2, uvl *uvlp, vms_vector *norm)
{
	fix			face_light;
	grs_bitmap	*bm;
	fix			reflect;
	g3s_uvl		uvl_copy[8];
	int			i;
	vms_vector	*pointlist[8];

	Assert(nv <= 8);

	for (i=0; i<nv; i++) {
		uvl_copy[i] = *(g3s_uvl *)&uvlp[i];
		pointlist[i] = &Segment_points[vp[i]].p3_vec;
	}
	...
	if (tmap2 != 0)
		bm = texmerge_get_cached_bitmap_maybe_vram( tmap1, tmap2 );
	else	{
...
ITCM_CODE void render_side(segment *segp, int sidenum)
{
...
ITCM_CODE void do_render_object(int objnum)
{
	#ifdef EDITOR
	int save_3d_outline;
	#endif
...
ITCM_CODE void render_start_frame()
{
	RL_framecount++;
...
ITCM_CODE g3s_codes rotate_list(int nv,short *pointnumlist)
{
	int i,pnum;
	g3s_point *pnt;
	g3s_codes cc;
...
extern int Automap_update;

// -----------------------------------------------------------------------------------
ITCM_CODE void render_segment(int segnum)
{
...
#if 0 // optimized out, no need to sort with hardware zbuffer
...
ITCM_CODE void render_frame(fix eye_offset)
{
	int start_seg_num;
...
*/
	for (nn=N_render_segs;nn--;) {
		...
		if (segnum!=-1 && (eye_offset>0 || visited[segnum]!=255)) {
