...
void hud_show_score_added(int x, int y);
...
void hud_printf (int *x, int *y, const char *format, ...)
{
	...
	if (*x < 0)
	{
		...
		*x = grd_curcanv->cv_w - w + *x;
	}
	if (*y < 0)
		*y = grd_curcanv->cv_h - 8 + *y;

	gr_printf(*x, *y, buffer);
}
...
//	gr_set_fontcolor(Color_0_31_0, -1);
	...
	hud_printf(&x, &y, score_str);

	if (score_time)
		hud_show_score_added(x, y + 8);
}
...
{	                                                                                                                                                                                                                                                             
	...
	gr_rect(last_x[0],y,SB_SCORE_RIGHT,y+GAME_FONT->ft_h);
...
void sb_show_score_added()
{
	...
	score_time -= FrameTime;
	if (score_time > 0) {
		if (score_display != last_score_display[0]) {
			gr_setcolor(BM_XRGB(0,0,0));
			gr_rect(last_x[0],SB_SCORE_ADDED_Y,SB_SCORE_ADDED_RIGHT,SB_SCORE_ADDED_Y+GAME_FONT->ft_h);
			last_score_display[0] = score_display;
		}
...
void play_homing_warning(void)
{
	...
	if (Players[Player_num].homing_object_dist >= 0) {
		...
		if (GameTime - Last_warning_beep_time[0] > beep_delay/2) {
			digi_play_sample( SOUND_HOMING_WARNING, F1_0 );
			Last_warning_beep_time[0] = GameTime;
		}
	}
}
	...
	if (Players[Player_num].homing_object_dist >= 0) {
		...
		if (GameTime & 0x4000) {
			if (Last_homing_warning_shown[0] != 1) {
				...
				Last_homing_warning_shown[0] = 1;
			}
		} else {
			if (Last_homing_warning_shown[0] != 0) {
				PIGGY_PAGE_IN(Gauges[GAUGE_HOMING_WARNING_OFF]);
				gr_ubitmapm( 7, 171, &GameBitmaps[Gauges[GAUGE_HOMING_WARNING_OFF].index] );
				Last_homing_warning_shown[0] = 0;
			}
		}
	} else if (Last_homing_warning_shown[0] != 0) {
		PIGGY_PAGE_IN(Gauges[GAUGE_HOMING_WARNING_OFF]);
		gr_ubitmapm( 7, 171, &GameBitmaps[Gauges[GAUGE_HOMING_WARNING_OFF].index] );
		Last_homing_warning_shown[0] = 0;
	}
...
void hud_show_keys(int x, int y)
{
	...
	if (x < 0)
		x = grd_curcanv->cv_w - 24 + x;
	if (y < 0)
		y = grd_curcanv->cv_h - 8 + y;

	if (Players[Player_num].flags & PLAYER_FLAGS_BLUE_KEY) {
		PIGGY_PAGE_IN(Gauges[KEY_ICON_BLUE]);
		gr_ubitmapm(x,y,&GameBitmaps[Gauges[KEY_ICON_BLUE].index]);
	}

	if (Players[Player_num].flags & PLAYER_FLAGS_GOLD_KEY) {
		PIGGY_PAGE_IN(Gauges[KEY_ICON_YELLOW]);
		gr_ubitmapm(x + 8,y,&GameBitmaps[Gauges[KEY_ICON_YELLOW].index]);
	}

	if (Players[Player_num].flags & PLAYER_FLAGS_RED_KEY) {
		PIGGY_PAGE_IN(Gauges[KEY_ICON_RED]);
		gr_ubitmapm(x + 16,y,&GameBitmaps[Gauges[KEY_ICON_RED].index]);
	}
}
...
//	gr_set_curfont( GAME_FONT );
//	gr_set_fontcolor(gr_getcolor(0,31,0),-1 );
	hud_printf(&x, &y,"%s: %i", TXT_ENERGY, f2ir(Players[Player_num].energy));
...
#else
			newdemo_record_player_energy(old_energy[0], energy);
#endif
			old_energy[0] = energy;
	 	}
	}
...
//	gr_set_fontcolor(gr_getcolor(0,31,0),-1 );
	...
	hud_printf(&x, &y, weapon_str);

	if (Primary_weapon == VULCAN_INDEX) {
#ifndef SHAREWARE
		if (Players[Player_num].primary_ammo[Primary_weapon] != old_ammo_count[0][0]) {
			if (Newdemo_state == ND_STATE_RECORDING)
				newdemo_record_primary_ammo(old_ammo_count[0][0], Players[Player_num].primary_ammo[Primary_weapon]);
			old_ammo_count[0][0] = Players[Player_num].primary_ammo[Primary_weapon];
		}
#endif
	}
}
...
#ifndef SHAREWARE
	if (Players[Player_num].secondary_ammo[Secondary_weapon] != old_ammo_count[1][0]) {
		if (Newdemo_state == ND_STATE_RECORDING)
			newdemo_record_secondary_ammo(old_ammo_count[1][0], Players[Player_num].secondary_ammo[Secondary_weapon]);
		old_ammo_count[1][0] = Players[Player_num].secondary_ammo[Secondary_weapon];
	}
#endif
}
...
void hud_show_invuln(int x, int y)
{
	...
	if (Players[Player_num].flags & PLAYER_FLAGS_INVULNERABLE) {
		if (((Players[Player_num].invulnerable_time + INVULNERABLE_TIME_MAX - GameTime) > F1_0*4) || (GameTime & 0x8000))
			hud_printf(&x, &y, "%s", TXT_INVULNERABLE);
	}
}
...
//	gr_set_fontcolor(gr_getcolor(0,31,0),-1 );
	if ( Players[Player_num].shields >= 0 )	{
		hud_printf(&x, &y,"%s: %i", TXT_SHIELD, f2ir(Players[Player_num].shields));
	} else {
		hud_printf(&x, &y,"%s: 0", TXT_SHIELD );
	}

	if (Newdemo_state==ND_STATE_RECORDING ) {
		int shields = f2ir(Players[Player_num].shields);

		if (shields != old_shields[0]) {		// Draw the shield gauge
#ifdef SHAREWARE
			newdemo_record_player_shields(shields);
#else
			newdemo_record_player_shields(old_shields[0], shields);
#endif
			old_shields[0] = shields;
		}
	}
}
...
//		gr_set_fontcolor(gr_getcolor(0,20,0),-1 );
		...
		if (x > 0)
			...
			x -= 12;
		hud_printf(&x, &y, "x %d", Players[Player_num].lives-1);
		gr_ubitmapm(x - 12,y,&GameBitmaps[Gauges[GAUGE_LIVES].index]);
	}
}
/*
void sb_show_lives()
{
	...
	if (Game_mode & GM_MULTI)
	{
		...
		gr_setcolor(BM_XRGB(0,0,0));
		gr_rect(last_x[0], y+1, SB_SCORE_RIGHT, y+GAME_FONT->ft_h);
		gr_set_fontcolor(gr_getcolor(0,20,0),-1);
		x = SB_SCORE_RIGHT-w-2;		
	...
	if (old_lives[0]==-1 || Players[Player_num].lives != old_lives[0]) {
...
//	gr_set_fontcolor(Color_0_31_0, -1 );
	...
	hud_printf(&x,&y,"%d:%02d", mins, secs);
#if 0
#ifdef PIGGY_USE_PAGING
	{
		char text[25];
...
void draw_player_ship(int cloak_state,int old_cloak_state,int x, int y)
{
	...
	gr_set_current_canvas(get_current_game_screen());
//	gr_bm_ubitbltm( bm->bm_w, bm->bm_h, x, y, x, y, &VR_offscreen_buffer.cv_bitmap, &grd_curcanv->cv_bitmap);
}
*/
...
void draw_numerical_display(int shield, int energy)
{
	...
	gr_printf((shield>99)?7:((shield>9)?5:7),15,"%d",shield);

	gr_set_fontcolor(gr_getcolor(25,18,6),-1 );
	gr_printf((energy>99)?7:((energy>9)?5:7),2,"%d",energy);
...
int draw_weapon_box(int weapon_type,int weapon_num)
{
	...
	if (weapon_num != old_weapon[weapon_type][0] && weapon_box_states[weapon_type] == WS_SET) {
		weapon_box_states[weapon_type] = WS_FADING_OUT;
		weapon_box_fade_values[weapon_type]=i2f(GR_FADE_LEVELS-1);
	}
		
	if (old_weapon[weapon_type][0] == -1) {
		draw_weapon_info(weapon_type,weapon_num);
		old_weapon[weapon_type][0] = weapon_num;
		old_ammo_count[weapon_type][0]=-1;
		drew_flag=1;
		weapon_box_states[weapon_type] = WS_SET;
	}

	if (weapon_box_states[weapon_type] == WS_FADING_OUT) {
		draw_weapon_info(weapon_type,old_weapon[weapon_type][0]);
		old_ammo_count[weapon_type][0]=-1;
		drew_flag=1;
		...
		if (weapon_num != old_weapon[weapon_type][0]) {
			weapon_box_states[weapon_type] = WS_FADING_OUT;
		}
		else {
			draw_weapon_info(weapon_type,weapon_num);
...
void draw_weapon_boxes()
{
	...
	if (drew) copy_gauge_box(&gauge_boxes[boxofs+0],&VR_offscreen_buffer.cv_bitmap);
	...
	if (!hostage_is_vclip_playing()) {
		...
		if (drew) copy_gauge_box(&gauge_boxes[boxofs+1],&VR_offscreen_buffer.cv_bitmap);
...
draw_hostage_gauge()
{
	...
	if (drew) {
		...
		old_weapon[1][0] = old_ammo_count[1][0] = -1;
	}
...
void draw_hud()
{
//	do_automap_multiscreen (53, 40 + 192, 140, 112);
//	do_automap_multiscreen (0, 0, 64, 64);

	softkey_render_ingame ();
/*
	//	Show score so long as not in rearview
	if ( !Rear_view && Cockpit_mode!=CM_REAR_VIEW && Cockpit_mode!=CM_STATUS_BAR) {
		hud_show_score(-1, 52 + 192);
	}
*/
...
void render_gauges()
{
	...
	if (Cockpit_mode == CM_FULL_COCKPIT) {
		...
		if (Players[Player_num].flags & PLAYER_FLAGS_INVULNERABLE) {
			...
			old_shields[0] = shields ^ 1;
		} else if (shields != old_shields[0]) {		// Draw the shield gauge
			if (Newdemo_state==ND_STATE_RECORDING ) {
#ifdef SHAREWARE
				newdemo_record_player_shields(shields);
#else
		...
		if (Players[Player_num].flags != old_flags[0]) {
			if (Newdemo_state==ND_STATE_RECORDING )
				newdemo_record_player_flags(old_flags[0], Players[Player_num].flags);
			draw_keys();
			old_flags[0] = Players[Player_num].flags;
		...
		if ((Game_mode & GM_MULTI) && !(Game_mode & GM_MULTI_COOP))
		{
			if (Players[Player_num].net_killed_total != old_lives[0]) {
				sb_show_lives();
				old_lives[0] = Players[Player_num].net_killed_total;
			}
		}
		else
		{
			if (Players[Player_num].lives != old_lives[0]) {
				sb_show_lives();
				old_lives[0] = Players[Player_num].lives;
			}
		}

		if ((Game_mode&GM_MULTI) && !(Game_mode & GM_MULTI_COOP)) {
			if (Players[Player_num].net_kills_total != old_score[0]) {
				sb_show_score();
				old_score[0] = Players[Player_num].net_kills_total;
			}
		}
		else {
			if (Players[Player_num].score != old_score[0]) {
				sb_show_score();
				old_score[0] = Players[Player_num].score;
			}
	...
	if (cloak != old_cloak[0] || cloak_fade_state || (cloak && GameTime>Players[Player_num].cloak_time+CLOAK_TIME_MAX-i2f(3))) {
		if (Cockpit_mode == CM_FULL_COCKPIT)
			draw_player_ship(cloak,old_cloak[0],SHIP_GAUGE_X,SHIP_GAUGE_Y);
		else
			draw_player_ship(cloak,old_cloak[0],SB_SHIP_GAUGE_X,SB_SHIP_GAUGE_Y);
