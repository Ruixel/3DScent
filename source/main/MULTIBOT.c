...
//																		loc += sizeof(shortpos);
	create_shortpos(&sp, Objects+objnum);
...
multi_send_boss_actions(int bossobjnum, int action, int secondary, int objnum)
{
	...
	*(short *)(multibuf+loc) = (short)bossobjnum;				loc += 2; // We won't network map this objnum since its the boss
	...
	*(short *)(multibuf+loc) = objnum;							loc += 2; // Objnum of object created by gate-in action
	if (action == 3) {
		*(short *)(multibuf+loc) = (short)Objects[objnum].segnum; loc += 2; // Segment number object created in (for gate only)
	}
	else
																		loc += 2; // Dummy
...
multi_send_create_robot_powerups(object *del_obj)
{
	...
	*(short *)(multibuf+loc) = (short)del_obj->segnum;		loc += 2;
	*(vms_vector *)(multibuf+loc) = del_obj->pos;		loc += 12;
	...
	{
		*(short *)(multibuf+loc) = (short)Net_create_objnums[i];
		loc += 2;
...
void
multi_do_claim_robot(ubyte *buf)
{
	short botnum;
	short remote_botnum;
	char pnum;
...
void
multi_do_release_robot(ubyte *buf)
{
	short botnum;
	char pnum;
...
void
multi_do_robot_position(ubyte *buf)
{
...
//	extract_shortpos(&Objects[botnum], (shortpos *)(buf+loc)); 
	...
	extract_shortpos(&Objects[botnum], &sp);
}

void
multi_do_robot_fire(ubyte *buf)
{
	// Send robot fire event
	int loc = 1;
...
void
multi_do_robot_explode(ubyte *buf)
{
...
void
multi_do_create_robot(ubyte *buf)
{
...
void
multi_do_boss_actions(ubyte *buf)
{
...
void
multi_do_create_robot_powerups(ubyte *buf)
{
...
void multi_robot_request_change(object *robot, int player_num)
{
	...
	if ( (robot_agitation[slot] < 70) || (MULTI_ROBOT_PRIORITY(remote_objnum, player_num) > MULTI_ROBOT_PRIORITY(remote_objnum, Player_num)) || (random() > 0x4400))
	{
