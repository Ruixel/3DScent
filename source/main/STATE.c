...
extern int ai_restore_state( ubyte **p );
...
//	m[0].type = NM_TYPE_TEXT; m[0].text = "\n\n\n\n";	
	for (i=0;i<NUM_SAVES; i++ )	{
		...
		if ( fp ) {
			...
			if ( !memcmp( id, dgss_id, 4 )) {
				...
				if (version >= STATE_COMPATIBLE_VERSION)	{
					...
					fread( desc[i], sizeof(char)*DESC_LENGTH, 1, fp );
					//rpad_string( desc[i], DESC_LENGTH-1 );
					m[i].type = NM_TYPE_MENU; m[i].text = desc[i];
					// Read thumbnail
					sc_bmp[i] = gr_create_bitmap(THUMBNAIL_W,THUMBNAIL_H );
					if (sc_bmp[i])
						fread( sc_bmp[i]->bm_data, THUMBNAIL_W * THUMBNAIL_H, 1, fp );
		...
		if (!valid) {
			strcpy( desc[i], TXT_EMPTY );
			//rpad_string( desc[i], DESC_LENGTH-1 );
			m[i].type = NM_TYPE_TEXT; m[i].text = desc[i];
		}
	}
...
//	choice = newmenu_do4( NULL, "Select Game to Restore", NUM_SAVES+1, m, state_callback, state_default_item+1, NULL, 380, -1, 1 );
	choice = newmenu_do4( NULL, "Select Game to Restore", NUM_SAVES, m, NULL, state_default_item, NULL, -1, -1, 1 );
...
//			pcx_file = get_briefing_screen( Next_level_num );
			pcx_file = get_briefing_screen( Current_level_num );
			if ( pcx_file != NULL )	{
				bmp.bm_data = NULL;
				if (pcx_read_bitmap( pcx_file, &bmp, BM_LINEAR, pcx_palette )==PCX_ERROR_NONE)	{
					grs_point vertbuf[3];
